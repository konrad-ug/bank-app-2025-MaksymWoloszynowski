...
class TestAccount:
    ...
    def test_account_creation_too_short_pesel(self):
        account = Account("John", "Doe", "490715123")
        assert account.pesel == "Invalid"

    def test_account_creation_too_long_pesel(self):
        account = Account("John", "Doe", "49071512328142314")
        assert account.pesel == "Invalid"
    
    def test_account_creation_no_promo_code(self):
        account = Account("John", "Doe", "49071512368", promo_code=None)
        assert account.balance == 0
    ...
    def test_after_1960_invalid_pesel_promo_code_bonus(self):
        account = Account("John", "Doe", "05271512342rwef", promo_code="PROM_2025")
        assert account.balance == 0

    def test_1960_promo_code_bonus(self):
        account = Account("John", "Doe", "60071512368", promo_code="PROM_2025")
        assert account.balance == 0
